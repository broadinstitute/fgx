...
- **`/datasets` rejects `format` but answers JSON anyway.** It accepts only `data_type`, `include_stats`, `resource`, so `fetch_json("/datasets")` 422s; and because the no-format response is JSON, `fetch_tsv` cannot parse it either. Neither helper works - call `client().get(f"{BASE}/datasets")` and parse `r.json()`, which is what nb07's `list_datasets` does.
