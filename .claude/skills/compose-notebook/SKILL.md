...
2. **Identify the endpoint.** Map the user's question to a `/api/v1/*` path. Don't guess -- if you're unsure, run `curl -fsS -H "Authorization: Bearer $FINNGENIE_TOKEN" "https://finngenie.fi/api/v1/<path>?format=json"` once to confirm the shape, then write the cell.
