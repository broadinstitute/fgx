...
- **Underscore-prefixed `@app.function` names get mangled per cell.** Marimo treats any name starting with `_` as cell-local and rewrites it at compile time to `_cell_<id>_<name>`. So a helper like `_client` defined in cell A and called from `fetch_tsv` in cell B fails at runtime with `NameError: name '_cell_<id>_client' is not defined`. The fix is the boring one: drop the leading underscore (`client`, not `_client`). Same rule applies to any future helper added with `@app.function` if it's called by another `@app.function`.
